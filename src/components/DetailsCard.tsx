...
const DetailsCard = () => {
  ...
  const {  pokemonDetails, errors, isLoading,  fetchDetails } = usePokemonStore();

  useEffect(() => {
    const controller = new AbortController();

    fetchDetails(detailId, controller.signal);
    ...
    <>
      ...
      <div className="bg-black/6 dark:bg-white/6 rounded-(--radius-cards) p-6 border border-black/10 dark:border-white/10 max-w-md mx-auto flex flex-col gap-6 text-stardust">
        ...
        {errors && errors.length > 0 && (
          <ErrorMessage messages={[errors[0] || 'Pokemon not found in the PokeAPI database']} />
        )}
        {!isLoading && !errors && !pokemonDetails && (
          ...
          <>
            <div className="text-center">
              {pokemonImage && (
                <div className="relative w-full aspect-square">
                  <PokemonImage src={pokemonImage} alt={pokemonDetails.name} />
                </div>
              )}
            ...
            <div className="grid grid-cols-2 gap-4 text-center">
              <div className="bg-black/4 dark:bg-white/4 p-2 rounded-lg">
                ...
                <p className="text-body-md font-medium">{(pokemonDetails.height / 10).toFixed(1)} м</p>
              </div>
              <div className="bg-black/4 dark:bg-white/4 p-2 rounded-lg">
                <p className="text-body-xs text-muted-text uppercase tracking-wider">Вес</p>
                <p className="text-body-md font-medium">{(pokemonDetails.weight / 10).toFixed(1)} кг</p>
              </div>
            </div>
